...
public class MainController implements Initializable {
    ...
    @FXML
    private ListView<String> memberDataList;
    @FXML
    ...
    private void loadCredits(ActionEvent event) {
        loadWindow("/library/assistant/ui/main/credits.fxml", "Our Team");
    }
    @FXML
    ...
    private void loadDelete(ActionEvent event) {
        loadWindow("/library/assistant/ui/delete/delete.fxml", "Delete Menu");
    }
    @FXML
    ...
    @FXML
    private void loadMemberInfo2(ActionEvent event) {
        ...
        try {
            while (rs.next()) {
                String mMemberID = id;
                String mMemberName = rs.getString("name");
                String mMemberMobile = rs.getString("mobile");
                String mMemberEmail = rs.getString("email");

                memberData.add("Member Name : " + mMemberName);
                memberData.add("Mobile No. : " + mMemberMobile);
                memberData.add("E-mail ID : " + mMemberEmail);
                
                String qu1 = "SELECT COUNT(memberID) as count FROM ISSUE WHERE memberID = '" + id + "'";
                ResultSet r1 = databaseHandler.execQuery(qu1);
                
                while (r1.next()) {
                    memberData.add("Books Issued : " + r1.getInt("count"));
                }

                memberData.add("Issued Books:-");
                memberData.add("\tBook ID\t\t\tBook Title");
                String qu2 = "SELECT bookID, title FROM ISSUE, BOOK WHERE ISSUE.memberID = '" + id + "' AND ISSUE.bookID=BOOK.id";
                ResultSet r2 = databaseHandler.execQuery(qu2);

                while (r2.next()) {
                    memberData.add("\t" + r2.getString("bookID") + "\t\t\t\t" + r2.getString("title"));           
                }           
        ...
        memberDataList.getItems().setAll(memberData);
    }
